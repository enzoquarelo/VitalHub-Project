    ...
    await api
      ...
      .then((response) => {
        ...
        if (selectedAgendadas) {
          ...
          filteredConsultas = response.data.filter(
            (consulta) => consulta.situacao.situacao === "Realizados"
          );
        } else if (selectedCanceladas) {
          filteredConsultas = response.data.filter(
            (consulta) => consulta.situacao.situacao === "Cancelados"
          );
        }
        ...
        <Container justifyContent={"start"}>
          ...
          >
            <SelectableButton
              ...
              onPress={() => handleButtonClick("Agendadas")}
            >
              <SelectableTitleButton fontSize={14} selected={selectedAgendadas}>
                Agendadas
              </SelectableTitleButton>
            </SelectableButton>
          ...
          {consultas.map((consulta, index) => {
            ...
            return (
              <Cards
                ...
                buttonSelected={buttonSelected} // Passe a propriedade buttonSelected para o componente Cards
                onCancelAppointment={() => {
                  setConsultaSelecionadaParaCancelamento(consulta);
